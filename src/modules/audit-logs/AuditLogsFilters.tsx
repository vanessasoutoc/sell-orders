...
import { AuditLogFilters, getCustomers, getOrderStatuses, getTransportTypes } from './auditLogsService';
...
export default function AuditLogsFilters({ filters, onChange }: Props) {
  const { data: statuses } = useQuery({ queryKey: ['order-statuses'], queryFn: getOrderStatuses });
  const { data: customers } = useQuery({ queryKey: ['customers-select'], queryFn: getCustomers });
  const { data: transportTypes } = useQuery({ queryKey: ['transport-types'], queryFn: getTransportTypes });
